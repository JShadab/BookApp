...
import { StudentModel } from './completedbooks.model';
@Component({
  selector: 'app-completedbooks',
  templateUrl: './completedbooks.component.html',
  styleUrls: ['./completedbooks.component.css']
})
export class CompletedbooksComponent implements OnInit {
  ...
  addtocompletedbooks(){
    ...
    this.api.completedbooksDetails().subscribe(res=>{
      alert("Student Detail Record Updated")
      let ref = document.getElementById('cancel')
      ref?.click();
      this.formValue.reset();
